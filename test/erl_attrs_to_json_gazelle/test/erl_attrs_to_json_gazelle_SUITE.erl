-module(erl_attrs_to_json_gazelle_SUITE).
...
conform_command(_) ->
    ...
    ?assertEqual(
       #{path => "/some/path/foo.erl",
         macros => ['TEST'],
         includes => ["/some/path"]},
       erl_attrs_to_json_gazelle:conform_command(Command1Decoded)),
    ...
    ?assertEqual(
       #{path => "/some/path/bar.erl",
         macros => [{'TEST', "1"}],
         includes => ["/some/path"]},
       erl_attrs_to_json_gazelle:conform_command(Command2Decoded)).
