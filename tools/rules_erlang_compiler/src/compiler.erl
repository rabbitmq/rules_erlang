...
-spec start_link(atom(), file:filename_all(), #{atom() := target()}, [string()], module_index(), inputs()) -> gen_server:start_ret().
        ...
        case length(Deps) of
            ...
            _ ->
                ...
                io:format(standard_error,
                        "        ~p will wait for ~p~n", [SrcRel, DepsRel])
        end,
        spawn_link(node(),
                   ?MODULE,
...
handle_cast({finished, Sender, From, Src, R}, #?MODULE{waiting = Waiting0, results = Results0} = S) ->
    ...
    {noreply, S#?MODULE{waiting = Waiting,
                        results = Results}}.
