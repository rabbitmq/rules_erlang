...
init([]) ->
    S = #?MODULE{
        src_analysis = ets:new(src_analysis, [set]),
        beam_file_contents = ets:new(beam_file_file_contents, [set])
    },
    {ok, S}.

terminate(shutdown, #?MODULE{
    src_analysis = AC,
    ...
    R =
        case ets:lookup(Table, Key) of
            [{Key, Hits, _, Module, ModuleBin, Warnings}] ->
                ets:insert(
                    Table, {Key, Hits + 1, erlang:monotonic_time(), Module, ModuleBin, Warnings}
                ),
...
handle_call({put_beam, Key, Contents}, _, #?MODULE{beam_file_contents = Table} = S) ->
    R =
        case Contents of
            {ok, Module, ModuleBin, Warnings} ->
                true = ets:insert_new(
                    Table, {Key, 0, erlang:monotonic_time(), Module, ModuleBin, Warnings}
                ),
                {ok, Module, ModuleBin, Warnings};
            E ->
                E
